...
//                                         <AvatarFallback className="bg-primary text-white text-sm" >{(suggestedUser?.username || 'U')?.slice(0, 2).toUpperCase()}</AvatarFallback>
//                                     </Avatar>
...
//                                 <AvatarFallback   className="bg-primary text-white text-sm" >{(selectedUser?.username || 'U')?.slice(0, 2).toUpperCase()}</AvatarFallback>
//                             </Avatar>
//                             <div className='flex flex-col'>
...
const ChatPage = () => {
  ...
  return (
    <div className="flex h-screen">
              ...
              >
                <Avatar className="w-14 h-14">
                  ...
                  <AvatarFallback className="bg-primary text-white text-sm">
                    {(suggestedUser?.username || 'U').slice(0, 2).toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div className="flex flex-col">
                  <span className="font-medium">{suggestedUser?.username}</span>
      ...
      {/* Chat Section */}
      {selectedUser && (
        <section className="flex-1 flex flex-col h-full w-full md:w-auto">
          {/* Back button for mobile */}
          <div className="flex items-center md:hidden p-2 border-b border-gray-300 bg-white">
            <Button
      ...
      {/* Empty state */}
      {!selectedUser && suggestedusers.length === 0 && (
        <div className="flex flex-col items-center justify-center mx-auto">
          <MessageCircleCode className="w-32 h-32 my-4" />
          <h1 className="font-medium">Your messages</h1>
          <span>Send a message to start a chat.</span>
