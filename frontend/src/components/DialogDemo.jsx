...
export function DialogDemo({ post }) {
  ...
  const postdeleteHandler = async () => {
    try {
      const res = await axios.delete(`${API_URL}/api/v1/post/delete/${post._id}`, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-full max-w-xs sm:max-w-sm rounded-xl p-0 overflow-hidden">
        <div className="flex flex-col text-sm divide-y">
          {user && user?._id !== post?.author._id && (
            <Button
              variant="ghost"
              className="text-red-500 font-semibold py-4 hover:bg-muted/40 rounded-none"
            >
              Unfollow
            </Button>
          )}
